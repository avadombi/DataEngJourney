...
# build the full url to access the data
city_name = "Portland"
...
def etl_weather_data(url):
    # make the request via the api to access the data
    rq = requests.get(url=url)  # Response [200] => Successful

    # get the data from the returned response
    data = rq.json()
